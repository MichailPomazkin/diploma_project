...
class NullTextInverter(BaseInverter):
    """
    Null-text Inversion с поддержкой пространственной маски.
    Реализует:
      - Физическое раздутие маски (Dilation) В ЛАТЕНТНОМ ПРОСТРАНСТВЕ
      - Soft Blending с временным управлением маской (cutoff_ratio)
      - Математически корректное Noise Erasure (амнезия) в зоне объекта (noise_strength)
    """
    ...
    def __init__(self, pipeline: StableDiffusionXLPipeline):
        ...
        with torch.no_grad():
            # Пустой промпт (без негативных ограничений, чтобы не ломать генерацию)
    ...
    # ЭТАП 1+2: DDIM-инверсия + градиентная оптимизация
    def invert(
            self,
            image: Image.Image,
            prompt: str,
        ...
        print("[Null-text] Этап 1: DDIM-инверсия (эталонная траектория)...")
        ...
        with torch.no_grad():
            ...
            prompt_embeds, _, pooled_prompt_embeds, _ = self.pipeline.encode_prompt(
                prompt=prompt, device=self.device, num_images_per_prompt=1,
                do_classifier_free_guidance=False
            )
        ...
        # Подготовка маски
        print(f"[Null-text] Этап 2: оптимизация null-text ({num_inner_steps} inner steps/шаг)...")
        ...
        if use_spatial_mask and mask is not None:
            mask_tensor = TF.to_tensor(mask).to(self.device)
            if mask_tensor.shape[0] > 1:
                mask_tensor = mask_tensor[0:1]

            mask_bg = (1.0 - mask_tensor).unsqueeze(0)
            ...
            print("[Null-text] Латентная маска загружена.")

        elif use_spatial_mask:
        ...
        for i, t in enumerate(forward_timesteps):
            ...
            for _ in range(num_inner_steps):
                ...
                if use_spatial_mask and prepared_mask_latent is not None:
                    mask_f = prepared_mask_latent.float()
                    diff = (pred_latent.float() - target_latent.float()) * mask_f
                    loss = (diff ** 2).sum() / (mask_f.sum() + 1e-8)
                else:
                ...
                if torch.isnan(loss) or torch.isnan(pred_latent).any() or torch.isinf(pred_latent).any():
                    print(f"  [Null-text] NaN/Inf на шаге {i}. Пропускаем оптимизацию.")
            ...
            if pred_latent is not None:
                if use_spatial_mask and prepared_mask_latent is not None:
                    mask_f = prepared_mask_latent.float()
                    diff = (pred_latent.float() - target_latent.float()) * mask_f
                    mse_err = ((diff ** 2).sum() / (mask_f.sum() + 1e-8)).item()
                else:
                    mse_err = F.mse_loss(pred_latent.float(), target_latent.float()).item()

                current_latent = target_latent.clone() if mse_err > 1.0 else pred_latent.detach()
            else:
                current_latent = target_latent.clone()

        return latent_noise, optimized_uncond_embeddings

    # ЭТАП 3: Реконструкция (Cutoff + Amnesia + Soft Blending)
    def reconstruct(
            self,
        ...
        if context is not None and len(context) != num_steps:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                h = latent_noise.shape[-2] * vae_scale_factor
                w = latent_noise.shape[-1] * vae_scale_factor
                time_ids = self.pipeline._get_add_time_ids(
                    (h, w), (0, 0), (h, w), dtype=prompt_embeds.dtype,
                    text_encoder_projection_dim=self.pipeline.text_encoder_2.config.projection_dim
                ).to(self.device)
                ...
                # Soft mask + Математически верная амнезия
                soft_mask = None
                if hasattr(self, 'spatial_mask') and self.spatial_mask is not None:
                    # Минимальное размытие (защита от двойной экспозиции)
                    ...
                    if noise_strength > 0.0:
                        ...
                        # Правильная дисперсия (math.sqrt)
                        obj_zone = latents * math.sqrt(1.0 - noise_strength) + pure_noise * math.sqrt(noise_strength)
                        latents = latents * mask_dt + obj_zone * (1.0 - mask_dt)

                cutoff_step = int(num_steps * cutoff_ratio)
                ...
                for i, t in enumerate(timesteps):
                    ...
                    if soft_mask is not None and i < cutoff_step:
                        target_lat = self.original_trajectory[i + 1].to(device=self.device, dtype=latents.dtype)
                        mask_dt = soft_mask.to(device=self.device, dtype=latents.dtype)
                        latents = latents * (1.0 - mask_dt) + target_lat * mask_dt

                    if i == cutoff_step and soft_mask is not None:
                        print(f"  [Null-text] Шаг {i}: маска отключена (свобода генерации).")
